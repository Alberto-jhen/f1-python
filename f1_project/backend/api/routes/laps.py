...
@router.get("/data/laps/{year}/{track}/{session}/{driver}", tags=["JSON_graphics_data"])
...
@router.get("/data/laps/distribution/{year}/{track}/{session}/{num_drivers}", tags=["JSON_graphics_data"])
...
@router.get("/data/qualy/overview/{year}/{track}", tags=["JSON_graphics_data"])
...
@router.get("/data/points/heatmap/{year}", tags=["JSON_graphics_data"])
async def get_points_heatmap_data(year: int):
    data = plotting_service.get_points_heatmap_json(year)
    if "error" in data:
        raise HTTPException(status_code=400, detail=data["error"])
    return data